...
class Network(AgentCheck):
    ...
    def _check_aix(self, instance):
        if self._collect_cx_state:
            netstat = subprocess.Popen(["netstat", "-f", "inet", "-n"],
                                        stdout=subprocess.PIPE,
                                        close_fds=True).communicate()[0]
        ...
        self._submit_regexed_values(netstat, BSD_TCP_METRICS)
        nets = sp.Popen(['netstat', '-in'], stdout=sp.PIPE, close_fds=True).communicate()[0]
        lines = nets.split("\n")
        net_if = set()
        AIX_METRICS = [
        ...
        for i in list(net_if):
            metrics={}
            enstat = sp.Popen(['entstat', i], stdout=sp.PIPE, close_fds=True).communicate()[0]
            iface = i.strip()
            lines=enstat.split("\n")
            for line in lines:
